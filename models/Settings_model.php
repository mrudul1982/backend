...
class Settings_model extends CI_Model
{
    ...
    public function category($id = null)
    {
        ...
        $this->where($this->tables['categories'] . '.id', $id);

        $this->users();

        return $this;
    }
public function category_deactivate($id)
    {
       $data = ['status' => '0'];
         $this->db->where('id', $id);
...
public function brands_activate($id){
    $data = ['status' => '1'];
         $this->db->where('id', $id);
       return $this->db->update('brands', $data);
        return false;
}
public function warehouses_activate($id){
    $data = ['status' => '1'];
         $this->db->where('id', $id);
       return $this->db->update('warehouses', $data);
        return false;
}
public function warehouses_deactivate($id){
     $data = ['status' => '0'];
         $this->db->where('id', $id);
       return $this->db->update('warehouses', $data);
        return false;
}
public function brands_deactivate($id){
    $data = ['status' => '0'];
         $this->db->where('id', $id);
       return $this->db->update('brands', $data);
        return false;
