...
class Products_model extends CI_Model
{
    ...
    public function addsubstitutes($data = [])
    {
        if ($this->db->insert('addsubstitutes', $data)) {
            $product_id = $this->db->insert_id();
            //return $this->getProductByID($product_id);
            return true;
        }
   ...
   public function addProduct($data, $items, $warehouse_qty, $product_attributes, $photos,$box_variants)
   {
    if ($this->db->insert('products', $data)) {
        $product_id = $this->db->insert_id();
        ...
        for($i=0;$i<count($box_variants);$i++)
        {

            $boxvariants = [
              'product_id' => $product_id,
              'name' =>$box_variants[$i]['name'],         
              'price' =>$box_variants[$i]['price'],
              'quantity' =>$box_variants[$i]['quantity'],
              'packsize' => $box_variants[$i]['packsize'],
              'ean' =>$box_variants[$i]['ean'],
          ];

          $this->db->insert('sma_box_variants', $boxvariants);
      }
...
public function previous_pricing($id){
    $this->db->select('product_price_history.*,products.name,users.username');
    $this->db->from('product_price_history');
    $this->db->join('products', 'product_price_history.product_id=products.id', 'left');
    $this->db->join('users', 'product_price_history.addedBy=users.id', 'left');
    $this->db->where('product_price_history.product_id', $id);
    $query = $this->db->get();
...
{
    $this->db->select('id,code,name,unit,cost,price,alert_quantity,image,category_id,subcategory_id,cf1,cf2, cf3,cf4,cf5,cf6 ,quantity,tax_rate,track_quantity,details,warehouse,barcode_symbology,file,product_details,tax_method,type ,supplier1,supplier1price,supplier2,supplier2price,supplier3,
  supplier3price,
  supplier4,
  supplier4price ,
  supplier5,
  supplier5price ,
  promotion as productDiscountApplicable,
  promo_price as discountedBoxPrice,
  start_date,
  end_date,
  supplier1_part_no,
  supplier2_part_no,
...
public function getSalesOptions($pid)
{
    $q = $this->db->get_where('sale_items', ['sale_id' => $pid]);
    if ($q->num_rows() > 0) {
        foreach (($q->result()) as $row) {
            $data[] = $row;
        }
...
public function get_product_code($products)
{
       // echo $products;exit();
    $this->db->select('code as text')
    ->where('id', $products);
    $q = $this->db->get('products');
    if ($q->num_rows() > 0) {
        foreach (($q->result()) as $row) {
...
public function check_product_code($products){
   $query = $this->db->get_where('addsubstitutes', ['products' => $products], 1);
...
public function updateProduct($id, $data, $items, $warehouse_qty, $product_attributes, $photos, $update_variants,$box_variants,$update_box_variants)
{
    if ($this->db->update('products', $data, ['id' => $id])) {
     ...
     for($i=0;$i<count($update_box_variants);$i++)
        {

            $updateboxvariants = [
              'product_id' => $id,
              'name' =>$update_box_variants[$i]['name'],         
              'price' =>$update_box_variants[$i]['price'],
              'quantity' =>$update_box_variants[$i]['quantity'],
              'packsize' => $update_box_variants[$i]['packsize'],
              'ean' =>$update_box_variants[$i]['ean'],
          ];
          $this->db->where('product_id',$id);
          $this->db->update('sma_box_variants', $updateboxvariants);
      }
...
public function deletesubstitutes($id)
{
    if ($this->db->delete('addsubstitutes', ['id' => $id])) {
        return true;
    }
    return false;
}
...
{
    if ($this->db->delete('manufacturers', ['id' => $id])) {
        return true;
    }
    return false;
}
public function getsubstitutesByID($id){

    $q = $this->db->get_where('addsubstitutes', ['id' => $id], 1);
    if ($q->num_rows() > 0) {
        return $q->row();
    }
    return false;
...
public function getsubstitutes_product($id){

    $q = $this->db->get_where('addsubstitutes', ['products' => $id], 1);
    if ($q->num_rows() > 0) {
        return $q->row();
    }
    return false;
...
public function updatesubstitutes($id,$data = []){

    if ($this->db->update('addsubstitutes', $data, ['id' => $id])) {
        return true;
    }
    return false;
...
public function updatemanufacturers($id,$data = []){
 if ($this->db->update('manufacturers', $data, ['id' => $id])) {
    return true;
}
return false; 
}
public function getmanufacturers($id){
    ...
    $q = $this->db->get_where('manufacturers', ['id' => $id], 1);
    if ($q->num_rows() > 0) {
        return $q->row();
    }
    return false;
...
public function manufacturers_deactivate($id){
    $data = ['status' => '0'];
    $this->db->where('id', $id);
    return $this->db->update('manufacturers', $data);
    return false;
}
public function manufacturers_activate($id){
    $data = ['status' => '1'];
    $this->db->where('id', $id);
    return $this->db->update('manufacturers', $data);
    return false;
