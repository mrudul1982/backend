...
class Auth_model extends CI_Model
{
    ...
    public function get_attempts_num($identity)
    {
        ...
        if ($qres !== false) { // Check if query executed successfully
            return $qres->num_rows();
        } else {
            // Log database error or handle accordingly
            return 0;
        }
    }
    return 0;
...
public function get_last_attempt_time($identity)
{
    ...
    public function login2($identity, $password, $remember = false)
    {
        ...
        if ($query->num_rows() === 1) {
            ...
            $password_matched = $this->hash_password_db($user->id, $password);
...
//echo 'hiii';exit();
        ...
        $data = [
            ...
            'last_login' => time(),
            'active'     => $active,
            // 'active'     => ($active ? 1 : (($manual_activation === true) ? 0 : 1)),
        ];
//print_r($data);exit();
        // if ($this->store_salt) {
        //     $data['salt'] = $salt;
