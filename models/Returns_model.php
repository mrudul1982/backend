...
class Returns_model extends CI_Model
{
    ...
    public function addReturn($data = [], $items = [])
    {
        ...
        $this->db->trans_start();
        if ($this->db->insert('sma_returns', $data)) {
    ...
    public function getProductNames($term, $limit = 5)
    {
     ...
     public function getProductNames2($term,$customer_id,$limit = 5)
    {
        $this->db->where("(product_name LIKE '%" . $term . "%' OR product_code LIKE '%" . $term . "%' OR  concat(product_name, ' (', product_code, ')') LIKE '%" . $term . "%')");
          $this->db->join('sma_sales', 'sma_sales.id=sma_sale_items.sale_id', 'left');

        $this->db->limit($limit);
        $q =$this->db->get_where('sma_sale_items', ['customer_id' => $customer_id], 1); 
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
...
public function getCustomer_Product($term, $limit = 10)
{
   ...
   public function getReturnItems($return_id)
    {
        $this->db->select('sma_return_items.*,sma_returns.date,sma_returns.reference_no,sma_products.size,sma_returns.customer_id')
               ->join('sma_returns', 'sma_returns.id = sma_return_items.return_id', 'left')
               ->join('sma_products', 'sma_products.id = sma_return_items.product_id', 'left')
            ->where('sma_return_items.return_id', $return_id)
            //->group_by('return_items.id')
            ->order_by('sma_return_items.id', 'asc');

        $q = $this->db->get('sma_return_items');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
