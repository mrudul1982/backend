...
class Site extends CI_Model
{
    ...
    {
        $setting=$this->db->query("select product_discount from  sma_settings")->row_array();
        if ($discount && ($order || $setting['product_discount'])) {
            $dpos = strpos($discount, '%');
            if ($dpos !== false) {
                $pds = explode('%', $discount);
                return number_format((((number_format($amount,2)) * (float) ($pds[0])) / 100), 2);
            }
    ...
    public function calculateOrderTax($order_tax_id = null, $amount = 0)
    {
        ...
        $setting=$this->db->query("select tax2 from  sma_settings")->row_array();
        if ($setting['tax2'] != 0 && $order_tax_id) {
            if ($order_tax_details = $this->site->getTaxRateByID($order_tax_id)) {
                if ($order_tax_details->type == 1) {
                    return number_format((($amount * $order_tax_details->rate) / 100), 2);
                }
                return number_format($order_tax_details->rate, 2);
            }
        }
        return 0;
    }
    ...
    public function calculateTax($product_details = null, $tax_details = null, $custom_value = null, $c_on = null) 
    {
        ...
        if ($tax_details && $tax_details->type == 1 && $tax_details->rate != 0) {
            if ($product_details && $product_details->tax_method == 1) {
                $tax_amount = number_format((($value) * $tax_details->rate) / 100, 2);

                $tax = number_format($tax_details->rate, 0) . '%';
            } else {
                $tax_amount = number_format((($value) * $tax_details->rate) / 100, 2);
            ...
            $tax_amount = number_format($tax_details->rate, 2);
            $tax = number_format($tax_details->rate, 0);
        }
    ...
    public function costing($items)
    {
        ...
        $item_tax = 0; // Initialize $item_tax here to avoid undefined variable notice
        if ($cpr->tax_rate) {
            $cpr_tax = $this->getTaxRateByID($cpr->tax_rate);
            if ($cpr->tax_method) {
                $item_tax = number_format((($combo_item->unit_price) * $cpr_tax->rate) / (100 + $cpr_tax->rate), 2);
                $net_unit_price = $combo_item->unit_price - $item_tax;
                $unit_price = $combo_item->unit_price;
            } else {
                $item_tax = number_format((($combo_item->unit_price) * $cpr_tax->rate) / 100, 2);
...
public function get_shop_sale_alerts()
{
    ...
    ->where('sales.shop', 1)->where('sales.sale_status', 'Waiting to be Invoiced')->where('sales.payment_status', 'paid')
...
public function getAllBrands()
{
 ...
 $q =$this->db->get_where('brands', ['status' => 1]);
 if ($q->num_rows() > 0) {
    foreach (($q->result()) as $row) {
        $data[] = $row;
    }
...
public function getAllsubproducts($sub_id)
{

 $this->db->where('id !=', $sub_id)->where('status',1);
...
{
    $this->db->where('parent_id', null)->or_where('parent_id', 0)->order_by('name')->where('status',1);
...
{
    $q = $this->db->get_where('manufacturers', ['status' => '1']);
    if ($q->num_rows() > 0) {
        foreach (($q->result()) as $row) {
            $data[] = $row;
        }
...
{
    $q = $this->db->get_where('sma_brands', ['id' => $id], 1);
    if ($q->num_rows() > 0) {
        return $q->row();
    }
    return false;
...
{
    $q = $this->db->get_where('sma_categories', ['id' => $id ,'status'=>1], 1);
    if ($q->num_rows() > 0) {
        return $q->row();
    }
    return false;
...
public function getCompanyByID($id)
{
  ...
  $q= $this->db->query("select sma_companies.*,sma_addresses.line1,sma_addresses.line2 ,sma_routes.route_number FROM `sma_companies` LEFT JOIN sma_addresses ON sma_companies.id=sma_addresses.company_id LEFT JOIN sma_routes ON sma_companies.  route=sma_routes.id WHERE sma_companies.id='$id'");
  if ($q->num_rows() > 0) {
    return $q->row();
}
return false;
...
{
    $q = $this->db->get_where('sma_customer_groups', ['id' => $id], 1);
    if ($q->num_rows() > 0) {
        return $q->row();
    }
    return false;
...
{
    $q = $this->db->get_where('sma_price_groups', ['id' => $id], 1);
    if ($q->num_rows() > 0) {
        return $q->row();
    }
    return false;
...
{
    $q = $this->db->get_where('sma_products', ['id' => $id], 1);
    if ($q->num_rows() > 0) {
        return $q->row();
    }
    return false;
...
{
    $q = $this->db->get_where('sma_sales', ['id' => $id],1);
    if ($q->num_rows() > 0) {
        return $q->row();
    }
    return false;
...
{
    $q = $this->db->get_where('sma_product_prices', ['price_group_id' => $group_id, 'product_id' => $product_id], 1);
    if ($q->num_rows() > 0) {
        return $q->row();
    }
    return false;
...
{
    $q = $this->db->get_where('sma_product_variants', ['product_id' => $product_id]);
    if ($q->num_rows() > 0) {
        foreach (($q->result()) as $row) {
            $data[] = $row;
        }
...
{
    $q = $this->db->get_where('sma_purchases', ['id' => $id], 1);
    if ($q->num_rows() > 0) {
        return $q->row();
    }
    return false;
...
public function getPurchasedItem($clause)
{
   // $orderby = empty($this->Settings->accounting_method) ? 'asc' : 'desc';
    ...
    $q = $this->db->get_where('sma_purchase_items', $clause);
    if ($q->num_rows() > 0) 
    {
        return $q->row();
    }
...
public function getPurchasedItems($product_id, $warehouse_id, $option_id = null, $nonPurchased = false)
{
    ...
    $this->db->select('sma_products.*,quantity as quantity_balance');
...
{
    $q = $this->db->get_where('sma_order_ref', ['ref_id' => '1'], 1);

    $setting=$this->db->query("select * from sma_settings")->row_array();
    ...
    if ($q->num_rows() > 0) 
    {
        ...
        switch ($field) {
            case 'so':
            $prefix = $setting['sales_prefix'];
            break;
            case 'pos':
            $prefix = isset($setting['sales_prefix']) ? $setting['sales_prefix'] . '/POS' : '';
            break;
            case 'qu':
            $prefix = $setting['quote_prefix'];
            break;
            case 'po':
            $prefix = $setting['purchase_prefix'];
            break;
            case 'to':
            $prefix = $setting['transfer_prefix'];
            break;
            case 'do':
            $prefix = $setting['delivery_prefix'];
            break;
            case 'pay':
            $prefix = $setting['payment_prefix'];
            break;
            case 'ppay':
            $prefix = $setting['ppayment_prefix'];
            break;
            case 'ex':
            $prefix = $setting['expense_prefix'];
            break;
            case 're':
            $prefix = $setting['return_prefix'];
            break;
            case 'rep':
            $prefix = $setting['returnp_prefix'];
            break;
            case 'qa':
            $prefix = $setting['qa_prefix'];
            case 'trip':
            $prefix = $setting['trip_prefix'];
            break;
            default:
            $prefix = '';
        }
        ...
        if ($setting['reference_format'] == 1) 
        {
            $ref_no .= date('Y') . '/' . sprintf('%04s', $ref->{$field});
        } elseif ($setting['reference_format'] == 2) 
        {
            $ref_no .= date('Y') . '/' . date('m') . '/' . sprintf('%04s', $ref->{$field});
        } elseif ($setting['reference_format'] == 3) 
        {
...
{
    $q = $this->db->get_where('sma_payments', ['sale_id' => $sale_id]);
    if ($q->num_rows() > 0) {
        foreach (($q->result()) as $row) {
            $data[] = $row;
        }
...
{
    $q = $this->db->get_where('sma_tax_rates', ['id' => $id], 1);
    if ($q->num_rows() > 0) {
        return $q->row();
    }
    return false;
...
{
    $q = $this->db->get_where('sma_units', ['id' => $id], 1);
    if ($q->num_rows() > 0) {
        return $q->row();
    }
    return false;
...
public function getUser($id = null)
{
    ...
    $q = $this->db->get_where('sma_users', ['id' => $id], 1);
   //echo $q;exit();
    if ($q->num_rows() > 0) {
        return $q->row();
    }
...
{
    $q = $this->db->get_where('sma_warehouses_products', ['product_id' => $product_id, 'warehouse_id' => $warehouse_id], 1);
    if ($q->num_rows() > 0) {
        return $q->row();
    }
    return false;
...
public function getWarehouseProducts($product_id, $warehouse_id = null)
{
    ...
    $q = $this->db->get_where('sma_warehouses_products', ['product_id' => $product_id]);
    if ($q->num_rows() > 0) {
        foreach (($q->result()) as $row) {
            $data[] = $row;
        }
...
public function item_costing($item, $pi = null)
{
    ...
    if ($setting['accounting_method'] != 2 && !$setting['overselling']) {
...
public function syncProductQty($product_id, $warehouse_id)
{
    ...
    if ($balance_qty!="")
    {
        if ($this->getWarehouseProducts($product_id, $warehouse_id)) {
            $this->db->update('sma_warehouses_products', ['quantity' => $wh_balance_qty], ['product_id' => $product_id, 'warehouse_id' => $warehouse_id]);
        } else {
            if (!$wh_balance_qty) {
                $wh_balance_qty = 0;
            }
            ...
            if ($product) {
                $this->db->insert('sma_warehouses_products', ['quantity' => $wh_balance_qty, 'product_id' => $product_id, 'warehouse_id' => $warehouse_id, 'avg_cost' => $product->cost]);
            } else {
                $this->db->insert('sma_warehouses_products', ['quantity' => $wh_balance_qty, 'product_id' => $product_id, 'warehouse_id' => $warehouse_id]);
            }
        }
        return true;
    }
...
public function syncSalePayments1($id,$cheque_status,$amount)
{
     //echo $cheque_status;exit();
    if($cheque_status ==''){
        $sale = $this->getSaleByID($id);
        ...
        if($cheque_status =='Cleared'){
            ...
            $this->db->update('sales', ['paid' => $amount , 'payment_status' => 'Paid','cheque_status'=>$cheque_status], ['id' => $id]);
            return true;
        }else {
         $this->db->update('sales', ['paid' => 0, 'payment_status' => 'Pending','cheque_status'=>$cheque_status], ['id' => $id]); 
...
public function updateReference($field)
{
    ...
    if ($q->num_rows() > 0) {
        $ref = $q->row();
        $this->db->update('sma_order_ref', [$field => $ref->{$field} + 1], ['ref_id' => '1']);
        return true;
    }
    return false;
}
...
private function getBalanceQuantity($product_id, $warehouse_id = null)
{
    ...
    $this->db->group_start()->where('status', 'received')->or_where('status', 'partial')->group_end();
    $q = $this->db->get('sma_purchase_items');
    if ($q->num_rows() > 0) {
        $data = $q->row();
        return $data->stock;
    }
...
public function getPromost2($id,$t){
    $today = date('Y-m-d');
    return $data = $this->db->query("select * FROM `sma_promos` WHERE amount <= $t and type ='invoice_amount' and start_date <= '$today' and end_date >= '$today' ORDER BY amount DESC")->row_array();

}
public function discount($t)
{

    $getPromost = $this->site->getPromost($t);

    
    if (count($getPromost)!='0') {
     foreach ($getPromost as $key) {
        $id = $key->id;
        $Promost = $this->site->getPromost2($id,$t);

    }

    if(count($Promost)!='0'){              
        $promos_id = $Promost['id'];
        $promos_amount = $Promost['amount'];
        $promos_percentage = $Promost['percentage'];
        $promos_gettype = $Promost['gettype'];
        $promos_getamount = $Promost['getamount'];
        $promos = array('id' =>$promos_id ,'amount' =>$promos_amount ,'percentage'=> $promos_percentage,'gettype' =>$promos_gettype ,'getamount'=> $promos_getamount);
        return $promos;
    }else{
        $promos= array();
        return $promos;
